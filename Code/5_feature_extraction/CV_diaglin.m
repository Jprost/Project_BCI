function [xroc_train_avg,yroc_train_avg,xroc_test_avg,yroc_test_avg,fisher_scores,ord_features, acc_train, acc_test] = CV_avg_performance_and_featScore_diaglin(kfold,features_matrix,nFeatKept, do_plot)
% Perform a kfold CrossValidation to have an unbiased accuracy estimation
% of a DiagLDA classifier, trying to discriminate MI task and offset (class 0
...
for iFold = 1:kfold 
    ...
    xroc_train_avg = padd_concat(xroc_train_avg,x_train);
    yroc_train_avg = padd_concat(yroc_train_avg, y_train);
    xroc_test_avg = padd_concat(xroc_test_avg, x_test);
    yroc_test_avg = padd_concat(yroc_test_avg, y_test);
